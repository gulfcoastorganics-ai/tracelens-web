export class Diagnostics {
  ...
  render({ tracking = 0, camera = "—", analysis = 0, quality = "high", rawConfidence = "—", stable = "—", area = "—", aspect = "—", motion = "—", rejection = "—", trace = null, appState = "—" } = {}) { if (!this.output) return; const traceText = trace ? ` · Trace ${trace.worker ? "worker" : "main"} ${trace.analysisResolution}px Cache ${trace.cacheHits}/${trace.cacheMisses}` : ""; this.output.textContent = `State ${appState} · FPS ${this.fps} · Tracking ${tracking}% · Raw ${rawConfidence}% · Stable ${stable} · Area ${area}% · AR ${aspect} · Motion ${motion} · Reject ${rejection} · ${camera} · Analysis ${Math.round(analysis)}ms · ${quality}${traceText}`; }
}
