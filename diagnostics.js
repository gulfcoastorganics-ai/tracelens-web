export class Diagnostics {
  ...
  render({ tracking = 0, camera = "—", analysis = 0, quality = "high", rawConfidence = "—", stable = "—", area = "—", aspect = "—", motion = "—", rejection = "—" } = {}) { if (!this.output) return; this.output.textContent = `FPS ${this.fps} · Tracking ${tracking}% · Raw ${rawConfidence}% · Stable ${stable} · Area ${area}% · AR ${aspect} · Motion ${motion} · Reject ${rejection} · ${camera} · Analysis ${Math.round(analysis)}ms · ${quality}`; }
}
