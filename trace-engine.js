...
import { normalizeTraceSettings } from "./trace-presets.js";
import { rankTraceLines, applyLinePriority, scoreTraceQuality, generateValueZones, generateTraceStages } from "./trace-analysis.js";
...
export class TraceEngine {
  ...
  async process(source, settings = {}, { preview = false } = {}) {
    ...
    try { sourceImage = await this.decode(source, resolution); } catch (error) { this.sourceCache.unmarkActive(sourceKey); throw error; }
    if (requestId !== this.lastRequest) { this.staleJobs += 1; this.sourceCache.unmarkActive(sourceKey); return { cancelled: true, key, preview }; }
    let result;
    try { result = this.worker ? await this.runWorker(requestId, imageDataToArray(sourceImage), normalized) : composeTrace(sourceImage, normalized); } catch (error) { this.sourceCache.unmarkActive(sourceKey); throw error; }
    this.sourceCache.unmarkActive(sourceKey); if (requestId !== this.lastRequest || result?.cancelled) { this.staleJobs += 1; return { cancelled: true, key, preview }; }
