...
export class ProjectLibrary {
  ...
  open() {
    ...
    this.dbPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = event => { const store = request.result.objectStoreNames.contains(STORE_NAME) ? request.transaction.objectStore(STORE_NAME) : request.result.createObjectStore(STORE_NAME, { keyPath: "id" }); if (event.oldVersion < 2) { const cursorRequest = store.openCursor(); cursorRequest.onsuccess = () => { const cursor = cursorRequest.result; if (!cursor) return; const value = cursor.value; cursor.update({ createdAt: value.createdAt || value.updatedAt || Date.now(), favorite: Boolean(value.favorite), archived: Boolean(value.archived), ...value }); cursor.continue(); }; } };
      request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error);
    });
    return this.dbPromise;
  }
  async put(project) { const db = await this.open(); return new Promise((resolve, reject) => { const tx = db.transaction(STORE_NAME, "readwrite"); tx.objectStore(STORE_NAME).put({ createdAt: Date.now(), favorite: false, archived: false, ...project }); tx.oncomplete = resolve; tx.onerror = () => reject(tx.error); }); }
  async all({ includeArchived = false } = {}) { const db = await this.open(); return new Promise((resolve, reject) => { const request = db.transaction(STORE_NAME).objectStore(STORE_NAME).getAll(); request.onsuccess = () => resolve(request.result.filter(project => includeArchived || !project.archived).sort((a, b) => b.updatedAt - a.updatedAt)); request.onerror = () => reject(request.error); }); }
